(() => {
    ...
    const warningCloseButton = document.querySelector("#warning-header-close");
    const nextElement = warningHeader.nextElementSibling;
    warningCloseButton.addEventListener("click", (() => {
        const warningHeaderHeight = warningHeader.offsetHeight;
        if (!warningHeader.classList.contains("warning-close")) {
            warningHeader.classList.add("warning-close");
            htmlElem.classList.add("warning-close");
            nextElement.style.marginTop = 100 - warningHeaderHeight + "px";
        }
    ...
    const scrollParam = header.getAttribute("data-scroll");
    window.addEventListener("scroll", (() => {
        const headerHeight = header.offsetHeight;
        let windowScrollTop = window.pageYOffset;
        if (header.classList.contains("_header-scroll") || windowScrollTop > scrollParam - 1) pageBody.style.paddingTop = headerHeight + "px"; else pageBody.style.paddingTop = 0;
    }));
    window["FLS"] = true;
    isWebp();
    addTouchClass();
